...
plot_gene_dendrogram_wgcna <- function(dat_expr_selected,
                                       module_colors_selected,
                                       out_file,
                                       main_title = "Temporal Cortex WGCNA Gene Dendrogram") {
  suppressPackageStartupMessages(library(WGCNA))
  
  gene_tree <- hclust(dist(t(dat_expr_selected)), method = "average")
  
  if (!is.null(names(module_colors_selected))) {
    module_colors_plot <- module_colors_selected[colnames(dat_expr_selected)]
  } else {
    if (length(module_colors_selected) != ncol(dat_expr_selected)) {
      stop("module_colors_selected length must match ncol(dat_expr_selected) when unnamed.")
    }
    module_colors_plot <- module_colors_selected
  }
  
  if (any(is.na(module_colors_plot))) {
    stop("Some module colors could not be matched to colnames(dat_expr_selected).")
  }
  
  module_colors_plot[module_colors_plot == "grey"] <- "grey90"
  module_colors_df <- data.frame(Module = module_colors_plot)
  
  grDevices::pdf(out_file, width = 14, height = 8, useDingbats = FALSE)
  
  WGCNA::plotDendroAndColors(
    dendro = as.dendrogram(gene_tree),
    colors = module_colors_df,
    groupLabels = "Module",
    main = main_title,
    dendroLabels = FALSE,
    hang = 0,
...
# ------------------------------------------------------------------------------
# Additional plot: Horizontal barplot of module counts
#   - Uses selected_gene_modules
#   - Saves module-wise selected gene counts
# ------------------------------------------------------------------------------

log_step("Saving module count horizontal barplot")

module_barplot_file <- file.path(params$out_dir, "temporal_wgcna_module_count_barplot.pdf")

module_count_table <- sort(table(selected_gene_modules), decreasing = FALSE)
module_names <- names(module_count_table)

module_bar_colors <- module_names
module_bar_colors[module_bar_colors == "grey"] <- "grey80"

grDevices::pdf(
  module_barplot_file,
  width = 11,
  height = max(6, length(module_count_table) * 0.45 + 2),
  useDingbats = FALSE
)

par(mar = c(5, 10, 4, 2))
bar_mid <- barplot(
  height = as.numeric(module_count_table),
  names.arg = module_names,
  horiz = TRUE,
  las = 1,
  col = module_bar_colors,
  border = NA,
  xlab = "Number of selected genes",
  main = "Selected gene counts by WGCNA module\n",
  cex.names = 0.95
)

text(
  x = as.numeric(module_count_table) + max(as.numeric(module_count_table)) * 0.01,
  y = bar_mid,
  labels = as.numeric(module_count_table),
  pos = 4,
  cex = 0.9
)
...
log_step(paste("Saved module count barplot to:", module_barplot_file))
